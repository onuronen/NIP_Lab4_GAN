...
def build_disc_true(true_disc_loss):
    plt.figure(1)
    plt.plot(range(len(true_disc_loss)), true_disc_loss, color='orange', label='true')
    plt.title("Discriminator True Loss Plot")
    plt.xlabel("# Epochs")
    plt.ylabel("# Loss")
    plt.savefig(OUTPUT_NAME + "_true_discriminator_loss_plot.png")


def build_disc_false(false_disc_loss):
    plt.figure(2)
    plt.plot(range(len(false_disc_loss)), false_disc_loss, color='blue', label='true')
    plt.title("Discriminator False Loss Plot")
    plt.xlabel("# Epochs")
    plt.ylabel("# Loss")
    plt.savefig(OUTPUT_NAME + "_false_discriminator_loss_plot.png")


def build_gen_graph(generator_loss):
    plt.figure(3)
    plt.plot(range(len(generator_loss)), generator_loss, color='green', label='false')
    plt.title("Generator Loss Plot")
    plt.xlabel("# Epochs")
    plt.ylabel("# Loss")
    plt.savefig(OUTPUT_NAME + "_generator_loss_plot.png")


def buildGAN(disc_ratio, gen_ratio, images, epochs = 40000, batchSize = 32, loggingInterval = 0):
    # Setup
    opt = Adam(lr = 0.0002)
    ...
    for epoch in range(epochs):
        for i in range(disc_ratio):
            ...
            loss_disc_true.append(advTrueLoss[0])
            advFalseLoss = adversary.train_on_batch(genImages, falseCol)
            loss_disc_false.append(advFalseLoss[1])
    ...
    build_disc_true(loss_disc_true)
    build_disc_false(loss_disc_false)
    build_gen_graph(all_gen_loss)
...
def main():
    ...
    (generator, adv, gan) = buildGAN(10,10, data, epochs = 15000, loggingInterval = 1000)
